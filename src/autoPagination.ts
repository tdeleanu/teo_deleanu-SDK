import {MethodSpec, RequestArgs, LotrResourceObject} from './Types.js';
...
class LotrIterator<T> implements AsyncIterator<T> {
  ...
  protected lotrResource: LotrResourceObject;
  constructor(
    firstPagePromise: Promise<PageResult<T>>,
    requestArgs: RequestArgs,
    spec: MethodSpec,
    lotrResource: LotrResourceObject
  ) {
    this.index = 0;
    ...
    this.spec = spec;
    this.lotrResource = lotrResource;
  }
  ...
  async iterate(pageResult: PageResult<T>): Promise<IteratorResult<T>> {
    if (
      !(
        pageResult &&
        pageResult.docs &&
        typeof pageResult.docs.length === 'number'
      )
    ) {
      throw Error(
        'Unexpected: Lotr API response does not have a well-formed `data` array.'
      );
    }

    const reverseIteration = isReverseIteration(this.requestArgs);
    if (this.index < pageResult.docs.length) {
      const idx = reverseIteration
        ? pageResult.docs.length - 1 - this.index
        : this.index;
      const value = pageResult.docs[idx];
      this.index += 1;

      return {value, done: false};
    } else if (pageResult.pages > pageResult.page) {
...
class ListIterator<T extends {id: string}> extends LotrIterator<T> {
  getNextPage(pageResult: PageResult<T>): Promise<PageResult<T>> {
    const reverseIteration = isReverseIteration(this.requestArgs);
    const lastId = getLastId(pageResult, reverseIteration);
    return this.lotrResource._makeRequest(this.requestArgs, this.spec, {
      [reverseIteration ? 'ending_before' : 'starting_after']: lastId,
    });
  }
}

class SearchIterator<T> extends LotrIterator<T> {
  getNextPage(pageResult: PageResult<T>): Promise<PageResult<T>> {
    if (pageResult.pages > pageResult.page) {
      throw Error(
        'Unexpected: Lotr API response does not have a well-formed `next_page` field, but `has_more` was true.'
      );
    }
    return this.lotrResource._makeRequest(this.requestArgs, this.spec, {
...
>(
  ...
  if (spec.methodType === 'search') {
    return makeAutoPaginationMethodsFromIterator(
      new SearchIterator(firstPagePromise, requestArgs, spec, lotrResource)
    );
  }
  if (spec.methodType === 'list') {
    return makeAutoPaginationMethodsFromIterator(
      new ListIterator(firstPagePromise, requestArgs, spec, lotrResource)
    );
  }
  return null;
};
...
function getLastId<T extends {id: string}>(
  ...
  const lastIdx = reverseIteration ? 0 : listResult.docs.length - 1;
  const lastItem = listResult.docs[lastIdx];

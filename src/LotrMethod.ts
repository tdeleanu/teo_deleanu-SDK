...
import {MethodSpec, LotrResourceObject} from './Types.js';
 ...
 *  the instance's path (e.g. 'movies' or 'quotes')
 ...
 *
 * <!-- Public API accessible via Lotr.LotrMethod.method -->
 */
export function lotrMethod(spec: MethodSpec): (...args: any[]) => Promise<any> {
  if (spec.path !== undefined && spec.fullPath !== undefined) {
    throw new Error(
      `Method spec specified both a 'path' (${spec.path}) and a 'fullPath' (${spec.fullPath}).`
    );
  }
  return function(this: LotrResourceObject, ...args: any[]): Promise<any> {
